...
for tt in range(5):
    clients_percentage = []
    time_complexity = []
    for P in [0.1]:  # Ensure proper indentation for the loop body
        ...
        mask_sh = results_df['car_id'].str.contains('SH', case=False, regex=False)
        mask_sz = results_df['car_id'].str.contains('SZ', case=False, regex=False)
        mask_bit = results_df['car_id'].str.contains('BIT', case=False, regex=False)
        # Extract data for each category
        df_sh = results_df[mask_sh]
        df_sz = results_df[mask_sz]
        df_bit = results_df[mask_bit]
        # Print descriptive statistics
        print("\nSH Category Statistics:")
        print(df_sh.describe())
        print("\nSZ Category Statistics:")
        print(df_sz.describe())
        print("\nBIT Category Statistics:")
        print(df_bit.describe())
