...
for tt in range(1):
    clients_percentage = []
    time_complexity = []
    for num_epochs in [128]:  # Ensure correct indentation in the loop body
        ...
        trainX_list = []
        trainY_list = []
        for car_id, ev_data in train_dict.items():
            # Collect all vehicle data
            trainX_list.append(torch.Tensor(ev_data.iloc[:, 1:].values).float())
            trainY_list.append(torch.Tensor(ev_data.iloc[:, 0].values).float())
        # Merge all vehicle data
        trainX_all = torch.cat(trainX_list, dim=0)
        trainY_all = torch.cat(trainY_list, dim=0)
        # Create global dataset
        dataset = Data.TensorDataset(trainX_all, trainY_all)
...
torch.save(final_model, '/home/ps/haichao/13_Fedaral_learning/trained_models/upper_model.pth')
# results_df.describe()
# %%
import pickle
...
with open('/home/ps/haichao/13_Fedaral_learning/results/fedupper_round.pkl', 'wb') as f:
